...
import { IndicatorState, IndicatorStatus } from "../models/Exchauster";
...
export class ClientConnection {
    ...
    subscribe(
        exchausterId: string,
        cb: (indicatorState: IndicatorState) => void
    ) {
...
export class FakeIndicatorStateGenerator {
    ...
    subscribe(cb: (state: IndicatorState) => void, timeInterval: number) {
        this.intervalId = setInterval(
            () =>
                cb({
                    ...
                    status: Math.floor(Math.random() * 5) as IndicatorStatus,
